...
async function releasePage(){const root=$('special');root.replaceChildren(el('p',{class:'muted'},'正在读取工作区…'));const result=await api('changes');state.files=result.files;state.selected=new Set();state.proof=null;root.replaceChildren(el('div',{class:'special-head'},el('h2',{},'待发布修改'),el('span',{class:'pill'},result.branch),button('刷新',()=>releasePage())));
  if(state.drafts.length){const section=el('div',{class:'notice'},el('strong',{},state.drafts.length+' 份草稿尚未写入官网'));const ids=new Set();for(const id of state.drafts){const cb=el('input',{type:'checkbox'});cb.onchange=()=>cb.checked?ids.add(id):ids.delete(id);section.append(el('label',{class:'file-row'},cb,state.records.find(r=>r.id===id)?.title||id));}section.append(button('写入所选草稿',()=>busy(async()=>{if(!ids.size)throw Error('请选择草稿');await job('apply',{ids:[...ids]});await refresh();await releasePage();toast('草稿已写入官网文件');})));root.append(section);}
...
async function route(){let group=location.hash.slice(1)||'home';if(!groups.some(g=>g[0]===group))group='home';if(state.dirty){const previous=state.group;confirmAction('切换栏目','当前修改尚未保存，确认后放弃编辑器内修改。',async()=>{state.dirty=false;await showGroup(group);});history.replaceState(null,'','#'+previous);return;}await showGroup(group);}
async function showGroup(group){state.group=group;history.replaceState(null,'','#'+group);nav();$('page-title').textContent=groups.find(g=>g[0]===group)[1];const special=['media','release','operations'].includes(group);$('workspace').hidden=special;$('special').hidden=!special;$('doc-actions').hidden=special;$('record-panel').hidden=['home','global','about','tools'].includes(group);$('search').value='';try{if(group==='media')await mediaPage();else if(group==='release')await releasePage();else if(group==='operations')operationsPage();else{renderRecords();const first=state.records.find(r=>r.group===group);if(first)await load(first.id);}}catch(e){toast(e.message,true);}}
$('save').onclick=()=>busy(async()=>{await save();toast('草稿已保存');});$('preview').onclick=()=>busy(buildPreview);$('apply').onclick=()=>busy(async()=>{if(state.dirty)await save();if(!state.doc.draft)throw Error('没有待写入的草稿');$('save-state').textContent='正在检查并写入…';await job('apply',{ids:[state.doc.id]});await refresh();await load(state.doc.id);toast('已写入官网文件，尚未推送');});
