...
tags: [身份安全, 企业安全, 检测工程]
dao_summary: "对手变了：从“无凭证的入侵者”变成“持有有效凭证的冒用者”。认证链路全绿、告警为零——人已经进来了。\n凭证有 L1-L6 六个层级，攻击者永远打当前防护下最弱的那层。约 80% 的数据泄露源于凭证泄露或滥用。\n身份就是新的边界。防御重心必须转向凭证全生命周期的持续证明。"
fa_summary: "方法论三步走：枚举攻击面 → 关联检测 → 按卡点推进。\n七条路径全景枚举，逐一映射 ATT&CK：AiTM、Infostealer、MFA 疲劳、SaaS 本地账号、签名密钥伪造、OAuth 恶意授权、恢复流程劫持。ITDR 侧用认证日志、特权访问、凭证暴露面、身份配置四类数据源，关联出九个信号。\n每条防御都写清落地卡点——不给正确但执行不了的清单。"
shu_summary: "落地以可复现为标准：九信号检测规则里，五条 KQL 可直接粘贴进 Microsoft Sentinel。\nAiTM 双 TLS 中继逐步拆解 Evilginx 如何拦截 Set-Cookie、MFA 全程“本人”参与为何仍然失效。\n公众号三篇层层递进：为何挡不住 → 不偷密码的入侵 → 零告警怎么抓。"
qi_summary: "外部依据收行业事实标准：MITRE ATT&CK 是七条路径技术映射的坐标系；FIDO2 / WebAuthn 给出凭证 L6 层级的答案；Microsoft Sentinel 承载 KQL 检测落地。\nB站讲解视频完整讲 Uber 案与零告警原理。每条标注来源、原创或转载与推荐星级。"
dao:
